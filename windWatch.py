...
def check_wind():
    ...
    while True:
        try:
            with open('/scripts/wxresult', 'r') as f:
                try:
                    data = json.load(f)
            ...
            print("Error opening file '/scripts/wxresult':", e)  # pickup json data from temp file
            time.sleep(2)
            continue
        break

    try:
        wind_speed = data['obs'][0]['wind_gust']
        if wind_speed >= 20:   # Set wind speed threshold here
            now = datetime.datetime.now()
            from_address = ""  # Set your FROM address here
            to_address = ""   # Set your destination address here
            subject = f"High Wind Alert: ({now.strftime('%m-%d-%Y %I:%M %p')})"
            message = f"THIS IS AN AUTOMATED WARNING - DO NOT REPLY \nWeather sensors have detected HIGH WIND GUSTS at or greater \
than {wind_speed}MPH in our neighborhood.  If outdoors or poolside, please consider moving to safe shelter. \
